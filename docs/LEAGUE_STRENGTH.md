...
θ is the maximum-likelihood estimate of the realised 1X2 outcome over **1,523
...
additive constant, so one league has to hold the origin.
    ...
    log-likelihood   −1522.77 at θ = 0  →  −1467.07 fitted
    2·ΔLL = 111.4 on 22 df, p far below 0.001
...
|---|---:|---:|---:|---|---:|
| Premier League (England) | **0** *(reference)* | 18.3 | 356 | mixed | 0.0 |
| Bundesliga (Germany) | −40.0 | 20.8 | 242 | uefa | −43.6 |
| La Liga (Spain) | −50.6 | 20.8 | 245 | uefa | −54.0 |
| Ligue 1 (France) | −66.1 | 23.1 | 196 | uefa | −69.5 |
| Serie A (Italy) | −70.6 | 20.6 | 243 | uefa | −74.4 |
| Jupiler Pro League (Belgium) | −84.1 | 26.2 | 146 | uefa | −88.9 |
| Primeira Liga (Portugal) | −102.3 | 26.1 | 148 | uefa | −106.9 |
| **Championship (England)** | −108.8 | 30.9 | 121 | league_cup | — |
| Superliga (Denmark) | −117.5 | 36.6 | 79 | uefa | −122.7 |
| **League One (England)** | −125.5 | 30.5 | 124 | league_cup | — |
| Eliteserien (Norway) | −129.6 | 31.2 | 112 | uefa | −135.0 |
| Ekstraklasa (Poland) | −137.3 | 36.2 | 80 | uefa | −144.9 |
| Eredivisie (Netherlands) | −148.3 | 26.1 | 156 | uefa | −152.4 |
| Super League (Greece) | −153.6 | 31.9 | 104 | uefa | −158.2 |
| Super League (Switzerland) | −161.8 | 36.3 | 88 | uefa | −166.7 |
| Süper Lig (Turkey) | −170.3 | 29.1 | 124 | uefa | −174.9 |
| Premiership (Scotland) | −175.2 | 32.5 | 99 | uefa | −180.2 |
| Bundesliga (Austria) | −195.2 | 36.0 | 91 | uefa | −200.2 |
| Liga I (Romania) | −202.8 | 46.0 | 48 | uefa | −208.0 |
| Allsvenskan (Sweden) | −224.0 | 39.3 | 73 | uefa | −240.9 |
| **League Two (England)** | −239.1 | 33.4 | 104 | league_cup | — |
| Premier Division (Ireland) | −313.2 | 67.1 | 27 | uefa | −319.0 |
| Veikkausliiga (Finland) | −329.7 | 55.1 | 40 | uefa | −335.3 |

*Written by `fitLeagueStrength.js` (run 32302831838, 19 Aug 2026 21:16 UTC).*
...
**THE TABLE IS THE SCRIPT'S OUTPUT NOW, and the first real run found two bugs.**
The values above were written by `fitLeagueStrength.js` against production, not
by the ad-hoc SQL that produced the first set. Getting there took two dry runs:

- **The season turned over in the wrong month.** `seasonOf` compared a
  ZERO-INDEXED `getUTCMonth()` to 7, putting the boundary in August where the
  repo's own `backfillSeasonFixtures.currentSeasonYear` puts it in JULY. Every
  July fixture was therefore matched against the clubs' previous divisions —
  and July is the busiest month in the UEFA calendar, 1,038 fixtures, more than
  any other. It surfaced as a uniform 2–3 point drift with the Championship 8.2
  points out; small enough to shrug at, systematic enough not to.
- **The fallback warning was false.** `loadEloParams` checked the JS field name
  against a map keyed by DB column, so it reported all three parameters falling
  back while using the fitted values correctly.

After both, the script and the original SQL agree to within **4.8 ELO points at
worst (Austria, 0.13 of its own standard error)**, with standard errors matching
to 0.2 across all 23 leagues. The small residual is accounted for: five fixtures
settled between the two runs (corpus 86,292 → 86,297, n 1,521 → 1,523), and the
script converges tighter — per-observation log-likelihood −0.96327 against the
SQL fit's −0.96389.
...
cadence is once per European season. Always dry-run first and compare: that is what caught both bugs above, and it
costs one workflow run.
