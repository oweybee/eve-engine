    ...
    https.request(
      ...
      res => {
        ...
        res.on('end', () => {
          ...
          // used to be indistinguishable from "this fixture has no odds
          // posted yet", so a quota-exhausted key silently looked like a
          // string of ordinary "no odds returned" skips instead of surfacing
          // as a loud, alertable failure (this is what caused the engine to
          // report success while writing zero rows for days). Treat any
          // non-empty `errors` as a hard failure so the caller sees it.
