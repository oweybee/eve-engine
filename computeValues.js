...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // `matchIds` also has to be chunked going IN: the scheduled backlog has grown
  // past ~9,500 rows (season-wide backfill, not just the near-term plan window),
  // and a single .in('match_id', matchIds) with that many UUIDs builds a request
  // URL long enough that the PostgREST gateway rejects it outright with a 400
  // ("Bad Request") before any freshness filter or pagination even applies —
  // which took the whole pipeline down, not just this query.
