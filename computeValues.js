...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked: it's a `uuid` PK, and matchData now
  // routinely holds thousands of rows since the whole-season backfill (was a
  // few hundred 'scheduled' fixtures at a time; is 9,000+ now) — a single
  // `.in('match_id', matchIds)` serialises to a query string PostgREST's
  // fronting proxy rejects outright with 400 Bad Request, which took down
  // every consumer of this function (computeValues, captureBaseline,
  // computeInplayValues, calibrate) simultaneously.
