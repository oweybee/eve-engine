...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked: the fleet now carries ~9-10k
  // 'scheduled' matches (fixtures scheduled months out), and passing all of
  // them in a single .in('match_id', matchIds) builds a multi-hundred-KB query
  // string that PostgREST rejects outright with 400 Bad Request — not a row
  // limit, a URL-length limit, so no amount of .range() paging avoided it.
