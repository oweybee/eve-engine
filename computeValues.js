...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds is also chunked before going into .in() — with the full slate
  // loaded many seasons ahead, matchIds routinely exceeds 9,000 entries, and
  // that alone produces a query string long enough for PostgREST to reject the
  // request outright (400 Bad Request) before the freshness filter or the
  // row-range pagination ever apply.
