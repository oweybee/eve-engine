...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // The 40-league expansion also pushed `matchIds` itself past what a single
  // `.in()` can carry in the querystring — with ~9,600 scheduled matches, the
  // encoded UUID list is hundreds of KB and PostgREST/the edge proxy reject it
  // outright with a 400 Bad Request, before the freshness filter or row paging
  // ever run. Chunk matchIds so each request's filter list stays a sane size.
  const freshCutoff = new Date(Date.now() - ODDS_MAX_AGE_HOURS * 3_600_000).toISOString();
  const oddsData = [];
  const ODDS_PAGE = 1000;
  const MATCH_ID_CHUNK = 200;
  for (let mFrom = 0; mFrom < matchIds.length; mFrom += MATCH_ID_CHUNK) {
    const matchIdChunk = matchIds.slice(mFrom, mFrom + MATCH_ID_CHUNK);
