...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds is also chunked before hitting .in(): on a busy slate (150+
  // scheduled matches) the UUID list alone pushes the GET request's query
  // string past the gateway's URL length limit, which PostgREST/Supabase
  // rejects outright as a 400 ("Bad Request") — failing every match's odds
  // fetch at once instead of just this one query.
