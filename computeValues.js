...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds is also chunked before hitting .in() — with the full scheduled
  // slate now spanning thousands of matches (40-league coverage landed in
  // #69), a single .in('match_id', matchIds) call produces a request URL long
  // enough to be rejected outright (400 Bad Request) by the Supabase edge
  // layer before it ever reaches PostgREST's own row cap.
