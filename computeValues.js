...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked: the backfilled schedule now carries
  // thousands of rows with status='scheduled' (season-ahead fixtures, not just
  // the near-term slate), so a single .in('match_id', matchIds) built a query
  // string long enough to get rejected outright (400 Bad Request) rather than
  // truncated — the same failure mode the row-pagination above was already
