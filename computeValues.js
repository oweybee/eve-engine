...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked before it goes into the request. With
  // the whole season backfilled, `statuses = ['scheduled']` now matches
  // thousands of fixtures (not just the near-term slate) — a single
  // .in('match_id', matchIds) built from all of them serialises into a query
  // string long enough that the gateway rejects it outright with a Bad Request,
  // before PostgREST ever gets to apply a row filter. That silently failed this
  // function (and therefore computeValues.js / captureBaseline.js) on every
  // single cycle once the scheduled backlog grew past that length.
