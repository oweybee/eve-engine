...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself is also chunked before hitting .in(): once planDay stopped
  // collapsing unmapped/mismatched-league fixtures into a shared placeholder
  // (4e5dad68), the 'scheduled' set grew large enough that the unchunked
  // `?match_id=in.(...)` query string exceeded PostgREST/Kong's URL length
  // limit and every call failed with a 400, taking computeValues and
  // captureBaseline down with it (matches planDay.js's CHUNK=500 pattern).
  const freshCutoff = new Date(Date.now() - ODDS_MAX_AGE_HOURS * 3_600_000).toISOString();
  const oddsData = [];
  const ODDS_PAGE = 1000;
  const MATCH_ID_CHUNK = 500;
