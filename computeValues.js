...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds is also batched below: a full-season fixture backfill can put
  // thousands of matches into `statuses`, and a `.in('match_id', matchIds)`
  // built from all of them produces a request URL PostgREST rejects with 400
  // Bad Request — which silently killed every compute cycle, not just the
  // oversized one.
  const freshCutoff = new Date(Date.now() - ODDS_MAX_AGE_HOURS * 3_600_000).toISOString();
  const oddsData = [];
  const ODDS_PAGE = 1000;
  const MATCH_ID_BATCH = 200;
  for (let mFrom = 0; mFrom < matchIds.length; mFrom += MATCH_ID_BATCH) {
    const idBatch = matchIds.slice(mFrom, mFrom + MATCH_ID_BATCH);
