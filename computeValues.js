...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  // backfill pushed status='scheduled' past 9,000 rows, and a single .in()
  // over every id serialises into a URL far past PostgREST's request-line
  // limit — the query came back 400 Bad Request on every match, not just the
  // ones past row 1000.
