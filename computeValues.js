...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // The match_id filter itself is also chunked: the scheduled-match set can run
  // into the thousands (matches table carries fixtures months out), and a single
  // .in('match_id', matchIds) with that many ids blows past PostgREST's request-
  // URL limit and comes back a blank 400 ("Bad Request"), aborting compute
  // outright for every match in the slate — not just the ones past row 1000.
