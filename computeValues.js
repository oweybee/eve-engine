...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds is also batched going IN to the filter: the full-season fixture
  // list now runs ~9.5k scheduled matches, and a single `.in('match_id', ids)`
  // with that many UUIDs builds a >300KB query string that the gateway rejects
  // outright with 400 Bad Request — which took down every caller of this
  // function (planDay, computeValues, captureBaseline, calibrate,
  // computeInplayValues) as the season calendar grew.
