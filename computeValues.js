...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  // 40-league expansion + whole-season backfill left `matches` carrying ~9.7k
  // scheduled rows, and a match_id=in.(<9700 uuids>) query string is ~350KB —
  // comfortably past the gateway's URL/header limit, so the request came back
  // as a flat 400 Bad Request and the entire compute+snapshot stage died before
  // it ever got to a real match. 200 ids/chunk keeps every request a few KB.
