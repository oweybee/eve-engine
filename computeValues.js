...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked before it goes into `.in()`: the
  // scheduled backlog has grown past ~9,600 rows, and a match_id IN-list that
  // size serialises into a query string long enough that PostgREST rejects the
  // request outright (400 Bad Request, no rows for anyone) rather than
  // returning a slow response — this was breaking every compute cycle.
