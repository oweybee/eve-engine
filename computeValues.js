...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself is also chunked: with the full-season backfill, `statuses`
  // can match 9,000+ scheduled fixtures, and .in('match_id', matchIds) inlines
  // every UUID into the request URL — a query string long enough that
  // PostgREST/the proxy in front of it rejects it outright (400 Bad Request),
  // which took the whole function down for every match, not just the excess
  // ones.
