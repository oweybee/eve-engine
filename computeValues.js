...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself is also chunked: the season backfill (#59/#60) can put
  // several thousand scheduled matches in play at once, and Supabase's JS
  // client serializes .in('match_id', matchIds) into the request URL — past
  // ~1-2k ids that URL exceeds the gateway's length limit and every request
  // fails with a flat "Bad Request", taking the whole engine down.
  ...
  for (let mFrom = 0; mFrom < matchIds.length; mFrom += MATCH_ID_CHUNK) {
    const idChunk = matchIds.slice(mFrom, mFrom + MATCH_ID_CHUNK);
