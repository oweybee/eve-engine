...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // The 40-league expansion put ~9,700 scheduled matches in play at once, and
  // `.in('match_id', matchIds)` puts every one of those ids on the request URL —
  // long enough to get the request itself rejected (400 Bad Request) before
  // PostgREST ever applies the freshness filter. Chunk the id list too, so the
  // URL stays a sane length regardless of how large the schedule grows.
