...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself also has to be chunked: the scheduled-match backlog grew
  // past ~2,000 fixtures, and a single `.in('match_id', matchIds)` with that
  // many UUIDs builds a request URL long enough that the API gateway rejects it
  // outright with a 400 — not truncated silently like the row cap above, but a
  // hard failure that took down every compute cycle (and captureBaseline.js,
