...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked before it goes into `.in()`: since the
  // 40-league expansion, `scheduled` matches number in the thousands, and a
  // single .in() with every id inflates the request URL past what
  // PostgREST/the gateway accepts, failing the whole call with a bare "Bad
  // Request" (no per-match detail) — silently blocking every consumer of this
  // function (computeValues, captureBaseline, computeInplayValues, calibrate)
  // on every run regardless of how few matches actually have fresh odds.
