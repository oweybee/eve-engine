...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself also has to be chunked before it goes into `.in()`: the
  // status='scheduled' filter above has no time bound, so a whole-season
  // backfill (thousands of future fixtures, all still 'scheduled') inflates
  // matchIds enough that the built URL is rejected outright — a bare "Bad
  // Request" from the gateway before PostgREST even parses it, not the usual
  // descriptive PostgREST error body. Chunking keeps each request's `.in()`
  // list bounded regardless of how many scheduled matches exist.
