...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself is also chunked below: the season backfill can leave
  // thousands of rows with status='scheduled' (games months out), and passing
  // all of them to a single .in() puts every id in the request URL — past a
  // few hundred that URL exceeds PostgREST/proxy request-size limits and the
  // whole call 400s, which starves every downstream computation.
