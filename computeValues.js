...
async function fetchMatchesForComputation(supabase, statuses = ['scheduled']) {
  ...
  //
  // matchIds itself must also be chunked: the whole-season fixture backfill
  // (#59/#60) can leave thousands of matches with status='scheduled' at once,
  // and a `.in('match_id', matchIds)` filter serialises every id into the
  // request URL — past a few hundred ids that line exceeds PostgREST's/the
  // gateway's URL length limit and the request comes back `Bad Request`,
  // aborting the whole compute cycle every time it runs.
