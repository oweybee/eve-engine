...
// `.in('match_id', matchIds)` serialises the whole id list into the request
// URL. computed_values started the run with 458 distinct match_ids on 9 Aug
// (up from a couple dozen when this bulk-read design was written) — a
// 458-UUID list is a ~17KB query string, past what Supabase's PostgREST
// gateway accepts, so every one of the three prefetch queries below was
// failing with a connection-level rejection. Because captureSnapshot.js was
// invoked as `node captureSnapshot.js 2>/dev/null || true` in engine.yml, the
// thrown error's message was discarded and the loop iteration silently
// produced zero snapshots — for 20+ hours before anyone noticed. Chunking
// keeps each request's id list bounded regardless of how large
// computed_values grows.
const ID_CHUNK_SIZE = 200; // ~7.4KB of UUIDs per chunk — comfortably under gateway URL limits
...
async function prefetchSnapshotExistence(supabase, matchIds, since7dIso) {
  const found = new Set();
  await Promise.all(chunk(matchIds, ID_CHUNK_SIZE).map(async ids => {
    ...
    for (const r of data ?? []) found.add(r.match_id);
  }));
  return found;
}
