...
--
-- current_tier()'s body was read directly off production with
-- pg_get_functiondef(oid) against pg_proc in this session, and is_plus_entitled
-- reproduces that same case expression verbatim, applied to p_user_id instead
-- of auth.uid(). It does NOT call current_tier() itself, because that function
-- reads auth.uid() and the reconciliation job runs as service_role with no
-- caller — it has to ask about MANY users, one at a time, from a context with
-- no session user at all.
--
-- SECURITY DEFINER, revoked from public/anon/authenticated: this must never
-- become a way for one member to query another member's subscription status.
-- Migration 107 is the reason the revoke names all three roles rather than the
-- two named roles alone — Postgres grants EXECUTE to PUBLIC on every function
-- it creates and anon/authenticated inherit through that grant even after
-- being revoked by name. Grant is to service_role only.
--
...
-- only reads telegram_links (from the reconciliation job added alongside this
-- migration) to decide who to kick or flag for the owner, and writes
-- *_status when it acts.
--
-- ── VERIFICATION ─────────────────────────────────────────────────────────────
--
-- APPLIED AND VERIFIED IN PRODUCTION (27 Aug 2026). Rehearsed once already in
-- a rolled-back transaction, then applied for real via the Supabase MCP against
-- the live MaxEdge Project — the same do $$ ... end $$ blocks below ran for
-- real this time and all passed:
--   • is_plus_entitled() agrees with the current_tier() case expression
--     above on EVERY row of the live profiles table.
...
--     the live profiles table, then deleting the probe row afterward.
...
--
-- Checked again independently of the apply call's own return value afterward:
-- telegram_links holds 0 rows (the probe's own cleanup held), RLS is on with
-- 3 policies, anon holds none of SELECT/INSERT/UPDATE/DELETE on the table nor
-- EXECUTE on is_plus_entitled, authenticated holds exactly select/insert/
-- update (no delete), and service_role holds EXECUTE on is_plus_entitled.
--
-- Still not done by this migration alone: eve-frontend's companion PR (branch
-- claude/sharp-hawking-rifvcy) still needs review/merge, the four Vercel env
-- vars, the bot's channel-admin promotion, and TELEGRAM_MEMBERSHIP_ENABLED
-- stays unset — this table and function existing cannot kick anyone on their
-- own.
--
-- Reversible:
...
-- own, so (select auth.uid()) is what makes it evaluate once per query rather
-- than once per row.
...
-- Supabase's default is every table privilege (including DELETE and TRUNCATE)
-- to BOTH roles on any table the migration role creates. The 7 Aug 2026
-- write-revoke narrowed every EXISTING table at the time; it did not — could
-- not — reach a table created afterward. Narrowed explicitly here instead,
-- asserted in section 6, not eyeballed.
