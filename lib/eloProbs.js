...
async function loadEloParams(supabase, { model = 'ELO_1X2', version = 'v1' } = {}) {
  ...
  const out = {
    drawAtParity: by.get('draw_at_parity') ?? DEFAULTS.drawAtParity,
    drawSpread:   by.get('draw_spread')    ?? DEFAULTS.drawSpread,
    homeAdv:      by.get('home_adv')       ?? DEFAULTS.homeAdv,
  };
  for (const [k, v] of Object.entries(out)) {
    if (v === DEFAULTS[k] && !by.has(k)) console.warn(`[eloProbs] ${k} fell back to the compiled value`);
  }
  return out;
}
