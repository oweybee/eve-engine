...
/**
 * DAILY LEDGER — the guard that stops a per-run cap being spent once per RUN.
 *
 * `planDailySpend` returns a DAY's allocation: `prematchCost` is
 * polls x cost x league-days, the whole day's intended spend. `ingestOddsApi`
 * read `prematchPolls` and used it as a PER-RUN league cap, so every tick was
 * free to spend the entire day's budget again. Measured 22 Aug 2026 with the
 * repaired sport-key table (36 leagues resolving, cost 2/request):
 *
 *   intended, per day                        2,880 credits
 *   actual, every 15 min (96 ticks/day)      6,912 credits/day -> 207,360/month
 *   actual, every 30 min (48 ticks/day)      3,456 credits/day -> 103,680/month
 *
 * against a 100,000 monthly allowance. So NO cron interval fixes this — at any
 * cadence worth running, a per-run cap spends the month by the middle of it and
 * the board goes dark for the rest, which is the failure this module's header
 * opens with. It never fired only because zero leagues resolved and zero
 * credits were spent; repairing the matcher is what made it live.
 *
 * The ledger is anchored on `x-requests-used`, the API's OWN counter, not on
 * our arithmetic — the same reason quotaFromHeaders beats our estimate
 * everywhere else. We store the counter's value at the first run of each UTC
 * day; today's spend is the difference, and it is correct across restarts,
 * concurrent runs and any run we failed to record.
 *
 * A DROP IN THE COUNTER IS A BILLING RESET, NOT A REFUND. The pool renews
 * mid-month on the subscription date, so `used` can go DOWN. Treated as a new
 * baseline; a negative spend would otherwise read as unlimited headroom.
 */
function dailyLedger({ storedDay = null, storedUsedAtDayStart = null, usedNow = null, today }) {
  if (usedNow == null || !Number.isFinite(usedNow)) {
    // No authoritative counter: keep the stored baseline and report unknown
    // rather than 0, because 0 spent reads as a full day's headroom.
    return { day: storedDay ?? today, usedAtDayStart: storedUsedAtDayStart, spentToday: null };
  }
  const rolled = storedDay !== today || storedUsedAtDayStart == null;
  const reset = storedUsedAtDayStart != null && usedNow < storedUsedAtDayStart;
  if (rolled || reset) return { day: today, usedAtDayStart: usedNow, spentToday: 0 };
  return { day: storedDay, usedAtDayStart: storedUsedAtDayStart, spentToday: usedNow - storedUsedAtDayStart };
}

/**
 * May we spend `cost` more credits today?
 * FAILS OPEN ON AN UNKNOWN LEDGER and closed on a known-exhausted one: without
 * the counter we still have `canSpend`'s hard reserve underneath, and refusing
 * every request because a header was missing would take the board down to
 * protect a budget.
 */
function withinDailyBudget({ spentToday, cost, ceiling }) {
  if (ceiling == null || !Number.isFinite(ceiling) || ceiling <= 0) return true;
  if (spentToday == null) return true;
