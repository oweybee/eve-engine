...
function buildCanonicalMap(rows, { floor = 0.92 } = {}) {
  ...
  for (const k of allKeys) {
    ...
    const supers = allKeys.filter(o => o !== k && isPrefix(kt, tokensByKey.get(o) ?? []));
    if (supers.length === 1) {
      resolvesTo.set(k, supers[0]);
      reasonByKey.set(k, 'prefix');
    } else if (supers.length > 1) {
      ambiguous.add(k);
      reasonByKey.set(k, `ambiguous prefix of ${supers.length}: ${supers.join(', ')}`);
    }
  }
...
module.exports = {
  stripAccents, tokens, normalize, canonicalKey, similarity, bestMatch,
