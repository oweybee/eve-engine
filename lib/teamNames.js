...
/**
 * Letters NFKD cannot decompose — see the same table in lib/lambdaBoard.
 * `ł`, `ı`, `ø`, `ß` are LETTERS, not a base plus a combining mark, so NFKD
 * leaves them whole and the ASCII strip downstream deletes them: "Wisła Płock"
 * became `wisapock` while our own ASCII row "Wisla Plock" became `wislaplock`,
 * and one club had two keys. THIS IS THE THIRD COPY of this normalisation in
 * the repo (lambdaBoard.clubKey and ensemble/scrapers/names.py are the others)
 * and engine.teamnames.test.js asserts all three agree, because three copies
 * that drift is the failure this codebase keeps paying for.
 */
const UNDECOMPOSABLE = /[łŁıİøØßđĐðÐþÞæÆœŒħĦŋŊŧŦĸ]/g;
const FOLD = {
  'ł': 'l', 'Ł': 'L', 'ı': 'i', 'İ': 'I', 'ø': 'o', 'Ø': 'O', 'ß': 'ss',
...
function stripAccents(s) {
  return String(s ?? '')
    .normalize('NFKD')
    .replace(/[̀-ͯ]/g, '')
    .replace(UNDECOMPOSABLE, ch => FOLD[ch] ?? ch);
}
