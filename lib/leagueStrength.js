...
/** A club placed only by its opponents needs at least this many rated ones. */
const DEFAULT_MIN_RATED_OPPONENTS = 4;
...
async function loadTeamScale(supabase) {
  const { data, error } = await supabase
    .from('team_scale')
    .select('canonical_key, theta_elo, source, n_rated_opponents');
...
function scaleFor(scale, key, minRatedOpponents) {
  const r = scale?.get?.(key);
  if (!r || !Number.isFinite(r.theta)) return null;
  if (r.source === 'opponents' && !(r.nRatedOpponents >= minRatedOpponents)) return null;
  return r;
}
...
function adjustPair(scale, {
  ...
  homeKey, awayKey, minRatedOpponents = DEFAULT_MIN_RATED_OPPONENTS,
} = {}) {
...
function comparable(scale, { competitionIsDomesticLeague, homeKey, awayKey,
                             minRatedOpponents = DEFAULT_MIN_RATED_OPPONENTS } = {}) {
...
/**
 * Why a pair was refused, for a log line or a withheld-reason column. Null when
 * it was not refused.
 */
function refusalReason(scale, { competitionIsDomesticLeague, homeKey, awayKey,
                                minRatedOpponents = DEFAULT_MIN_RATED_OPPONENTS } = {}) {
  if (typeof competitionIsDomesticLeague !== 'boolean') {
    return 'competitionIsDomesticLeague was not supplied';
  }
  if (competitionIsDomesticLeague) return null;
  const bad = [];
  for (const [side, key] of [['home', homeKey], ['away', awayKey]]) {
    const r = scale?.get?.(key);
    if (!r || !Number.isFinite(r.theta)) {
      bad.push(`${side} club has no place on the global scale`);
    } else if (r.source === 'opponents' && !(r.nRatedOpponents >= minRatedOpponents)) {
      bad.push(`${side} club is placed only by ${r.nRatedOpponents} rated opponent(s)`);
    }
  }
...
module.exports = {
  ...
  loadTeamScale, loadLeagueStrength, scaleFor, comparable, adjustPair, refusalReason,
};
