...
function priceSecondaryMarkets(match, consensus, homeStats, awayStats, refStats) {
  ...
  if (corners && corners.line != null) {
    ...
    if (lambda != null) {
      ...
      const mkt = devigTwoWay(corners.over, corners.under);
      result.corners = {
        line: corners.line, lambda, modelProb: pOver,
        over:  side(corners.over,  pOver,     mkt?.over  ?? null),
        under: side(corners.under, 1 - pOver, mkt?.under ?? null),
      };
    }
  }
  ...
  if (bookings && bookings.line != null) {
    ...
    if (lambda != null) {
      ...
      const mkt = devigTwoWay(bookings.over, bookings.under);
      result.bookings = {
        line: bookings.line, lambda, modelProb: pOver,
        over:  side(bookings.over,  pOver,     mkt?.over  ?? null),
        under: side(bookings.under, 1 - pOver, mkt?.under ?? null),
      };
    }
  }
