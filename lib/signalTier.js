 ...
 * PRIME / WATCH / NOTED / NOISE. Two of the words were shared and neither meant
 ...
 */
const LABELS = Object.freeze(['PRIME', 'WATCH', 'NOTED', 'NOISE']);

/** Score cutoffs, strongest first. 65 is the selection boundary BY
 *  CONSTRUCTION: maxedge_score() reads 100·(1 − 0.35^(z/z_gate)), which is
 *  exactly 65 at z = z_gate. Nothing chooses it. */
const BAND_MIN = Object.freeze({ PRIME: 65, WATCH: 40, NOTED: 25, NOISE: 0 });

/**
 * The rung for a MaxEdgeScore. Mirrors `maxedge_band(integer)` in migration 048.
 *
 * Returns null for a score that does not exist — never NOISE. A null MXS means
 ...
 * frontend renders the first as an absent badge and the second as ◇ NOISE.
 *
 * @param {number|string|null|undefined} mxs
 * @returns {'PRIME'|'WATCH'|'NOTED'|'NOISE'|null}
 */
function bandFor(mxs) {
  ...
  if (n >= BAND_MIN.NOTED) return 'NOTED';
  return 'NOISE';
}
