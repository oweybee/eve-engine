...
// LOWERED 4% -> 3% (owner, 22 Aug 2026), to match `f(edge)`'s plateau.
//
// WHY IT HAD TO MOVE. The plateau was loosened to 3% the day before, and this
// floor stayed at 4% — so a row with a 3.1% edge kept its FULL score (f = 1.00
// on the plateau) and its full rung, while this box declined it. The homepage
// drew the result: `◆ PRIME · 65` under a header reading NOTHING BACKED TODAY.
// The two ladders now meet at one number and `EDGE_EFFICIENCY.plateauFrom`
// READS THIS CONSTANT, so the gap cannot reopen by editing one of them.
//
// WHAT IT COSTS, STATED. The 4% floor's evidence is the band above it: odds
// 1.40-3.00 / edge 4-10% returns +15.71% over 143 settled bets, z 1.64. The
// band this admits — 3-4% at the same prices — is +6.26% over 38 bets, z 0.35:
// positive, and not distinguishable from zero. So this widens what the product
// SUGGESTS, broadcasts to Telegram and alerts on, onto a cohort with no
// measured edge. Owner's call, made with that number in front of them.
const PRIME_EDGE_MIN  = 0.03;
 ...
 */
const LABELS = Object.freeze(['PRIME', 'WATCH', 'SLIGHT', 'TRACE', 'NIL']);
...
const BAND_MIN = Object.freeze({
  PRIME: 65, WATCH: 41, SLIGHT: 23, TRACE: 10, NIL: 0,
});
...
function isBacked(mxs) {
  // READ THE CUTOFF, NOT THE WORD. This has been the 65 line throughout — it
  // was PRIME(88)+STRONG(65) under the six-rung ladder and it is PRIME(65)
  // alone under the five-rung one, and the SET OF ROWS is identical. Reading
  // BAND_MIN is what made the 21 Aug relabel a vocabulary change rather than a
  // silent move of the selection bar.
  const band = bandFor(mxs);
  return band !== null && BAND_MIN[band] >= BAND_MIN.PRIME;
}
