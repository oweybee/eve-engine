 ...
 */
// plateauFrom LOOSENED 4% -> 3% (owner, 22 Aug 2026). f(edge) was built
// against stale HIGH edges, but the low ramp is what was actually biting: of
// 41 open published signals that morning only 4 scored 65+, and the two that
// lost PRIME did so at 2.47% and 3.32% edge — below the old plateau, not above
// the knee. The knee, shoulder and trap floor are UNCHANGED, so "nothing over
// a 10% edge can be PRIME" is still the same theorem. Mirrors
// eve-frontend/lib/maxedge.ts; keep them in step.
// plateauFrom IS the eligibility box's edge floor, read from signalTier rather
// than repeated (22 Aug 2026). Typed twice, they drifted within a day and put
// a PRIME mark on a row every action declined.
const EDGE_EFFICIENCY = Object.freeze({
  plateauFrom: THRESHOLDS.PRIME_EDGE_MIN, plateauTo: 0.09,
  kneeAt: 0.10, knee: 0.65,
  trapFrom: 0.12, shoulderEnd: 0.40,
  floor: 0.50, trap: 0.25,
});

function edgeEfficiency(edge) {
  // `Number(null)` IS 0 AND 0 IS FINITE, so a bare Number() here files an
  // UNKNOWN edge as a zero one and hands it the 0.50 floor — a penalty invented
  ...
  if (e <= 0)            return K.floor;
  if (e < K.plateauFrom) return K.floor + (e / K.plateauFrom) * (1 - K.floor);
  if (e <= K.plateauTo)  return 1;
  if (e <= K.kneeAt) {
    const t = (e - K.plateauTo) / (K.kneeAt - K.plateauTo);
    return 1 - t * (1 - K.knee);
  }
  if (e <= K.trapFrom) {
    const t = (e - K.kneeAt) / (K.trapFrom - K.kneeAt);
    return K.knee - t * (K.knee - K.shoulderEnd);
  }
  return K.trap;
}
