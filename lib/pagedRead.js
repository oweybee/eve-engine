...
/**
 * Keyset paging — walk a table on a UNIQUE column with `key > last`, never an
 * OFFSET.
 *
 * WHY NOT `pageAll`. `.range(from, to)` is OFFSET/LIMIT, and OFFSET has two
 * costs on a table that is being WRITTEN while it is read. Each page re-runs
 * the whole filtered scan and then discards `from` rows of it — so page eight
 * of a 24-hour `odds` window sorts the same 7,000 rows eight times over. And a
 * row inserted between two pages shifts every later offset by one, so a walk
 * ordered by a uuid (which is not time-ordered) can skip a row or return one
 * twice. The in-play worker inserts into `odds` every 30 seconds while
 * computeValues is reading it, so both are live rather than theoretical.
 *
 * `key > last` re-uses the same filtered scan each page but sorts only what
 * is still ahead, and an insert between pages lands either behind the cursor
 * (missed, exactly as it would have been by the snapshot the read started
 * from) or ahead of it (returned once). Neither page can be wrong about a row
 * it already returned.
 *
 * THE KEY MUST BE UNIQUE, for the same reason `pageAll` says so: a tie at a
 * page boundary would be skipped by `>`. A primary key is the right key; it
 * does not have to be time-ordered.
 *
 * `limit` rather than `range`, so the request is the page size and nothing
 * else. PostgREST's 1,000-row cap and a page of exactly 1,000 agree, so a
 * short page is still the unambiguous end-of-data signal.
 *
 * @param {() => object} query builds a FRESH query — it is called once per page
 * @param {string} key     a unique column to page on
 * @param {string} label   names the read in an error, so a throw is placeable
 */
async function pageByKey(query, key, label = 'keyset read') {
  const out = [];
  let last = null;
  for (;;) {
    let q = query();
    ...
    if (last === undefined || last === null) {
      throw new Error(`${label}: page key '${key}' is not in the selected columns`);
    }
  }
  return out;
}

module.exports = { pageAll, pageByKey, inChunks, IN_CHUNK, PAGE_SIZE };
