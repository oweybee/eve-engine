...
const BOOKMAKER_LABEL = Object.freeze({
  // API-Football's aggregated LIVE feed, not a book a reader can hold an
  // account with — and it is the ONLY source that prices an in-play row, so
  // this is the name every in-play alert prints. Without an entry here
  // `bookmakerKey` slugifies it to 'apifootballlive', which reads as a
  // bookmaker nobody has heard of; naming it as a feed is the honest form.
  'apifootball_live': 'Live feed (aggregated)',
  bet365: 'Bet365',
...
const ALIASES = Object.freeze({
  // `bookmakerKey` must send a LABEL back to its own key — an invariant
  // engine.bookmakers.test.js pins, and one a readable label for
  // 'apifootball_live' cannot satisfy on its own (no display name slugifies to
  // a string containing an underscore). Both the lowercased label and its slug
  // are mapped, because bookmakerKey tries the raw lowercase first and only
  // slugifies as a fallback. Without this the display name resolves to a key
  // that matches no row, so a bet recorded from an in-play alert would store a
  // bookmaker nothing can look up. Caught by that test the first time the
  // label was added, and the first fix keyed only the slug and still failed.
  'live feed (aggregated)': 'apifootball_live',
  'livefeedaggregated':     'apifootball_live',
