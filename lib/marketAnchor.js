...
/**
 * Shin-de-vigged probabilities for a COMPLETE market, keyed by selection.
 *
 * Generic on purpose: this platform de-vigs two different price vectors for two
 * different questions, and both deserve the same solver rather than a second
 * hand-rolled one.
 *
 *   the ANCHOR's prices     → what the sharpest book believes. Fair value.
 *   the BETTABLE PANEL's
 *   best prices per outcome → what the market a reader can actually bet into
 *                             believes, margin removed. This is `market_prob`.
 *
 * A market is de-vigged as a complete set or not at all — `shinDevig` rejects a
 * partial vector rather than normalising two legs of a three-way book to 1 and
 * pricing the draw at zero.
 *
 ...
 */
function devigProbs(pricesBySelection, order) {
  ...
  // A VECTOR WITH NO MARGIN LEFT IS NOT A MARKET, AND MUST NOT BECOME A
  // `market_prob`. `shinDevig` still returns probabilities for an overround at
  // or below 1 — it normalises and raises the `arbitrage` flag — and that is the
  // right call for a solver, whose job is to give the best estimate available.
  // It is the wrong call here. Normalising a sub-1 vector scales every leg UP,
  // so `market_prob x detected_odds` lands ABOVE 1: the de-vigged "fair" line
  // says the outcome is likelier than the price you can take, and every leg
  // clears the EV threshold at once. Measured on 7 Aug 2026 across the live
  // ledger, 25 h2h rows and 121 totals/BTTS rows carried exactly that, and 73
  // fixtures had BOTH sides of a two-way market flagged as value.
  //
  // A real overround is never below 1. Below 1 means one leg is stale, from a
  // different moment, or from a feed that does not belong in the vector — so
  // the honest answer is that this market cannot be de-vigged, not a number
  // that reads like value. `scoreSignal` refuses a null `market_prob`, which
  // makes the row unscorable rather than scored against an impossible market.
  if (r.arbitrage) {
    return {
      fair: null, fairPrices: null, overround: r.overround, z: r.z,
      arbitrage: true,
      reason: `no margin to remove — overround ${r.overround.toFixed(4)} is at or below 1`,
    };
  }
  const fair = {};
  const fairPrices = {};
 ...
 * The anchor case of `devigProbs` — same solver, named for the job it does here.
 */
function fairFromAnchor(pricesBySelection, order) {
  return devigProbs(pricesBySelection, order);
}

/**
 * The de-vigged market probability for ONE selection, from the complete vector
 * of prices it was detected against. Returns null when the market cannot be
 * de-vigged, and null is the whole point: `scoreSignal` refuses to score a row
 * whose market probability it cannot state honestly, rather than falling back
 * to `1 / odds` and quietly reintroducing the margin.
 *
 * @param {Record<string, number|string>} pricesBySelection
 * @param {string[]} order
 * @param {string} selection which key's probability to return
 * @returns {number|null}
 */
function devigMarketProb(pricesBySelection, order, selection) {
  const r = devigProbs(pricesBySelection, order);
  const p = r.fair?.[selection];
