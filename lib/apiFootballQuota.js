 ...
 *
 * ── HEADER NAMES ARE THE VENDOR'S AND ARE NOT VERIFIED HERE ─────────────────
 *
 * `API_FOOTBALL_KEY` is not available in the environment this was written in, so
 * these four names come from the v3 documentation and are not corroborated by a
 * live response — the same footing lib/oddsApi.js shipped on. They are read
 * case-insensitively and every one of them is optional: an unrecognised header
 * yields nulls, `report()` stores nothing, and ingestion is untouched. **Check
 * `npm run api-quota` after the first real run** — a stored `limitDay` of 75000
 * is the confirmation, and all-nulls means the names are wrong.
 *
