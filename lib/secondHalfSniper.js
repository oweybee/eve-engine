...
const { bestTotalsByLine, inplayEdge, isBackablePrice, INPLAY_MAX_ODDS } = require('./inplay');
const { adjustLambdaForCards } = require('./inplayState');
...
function sniperCandidates(match, baseline, opts = {}) {
  ...
  // Above 3.00 this stage has produced 265 above-max rejects and ZERO
  // candidates inside the band, over the whole inplay_market_series record —
  // the plainest form of the favourite-longshot bias lib/inplay measures.
  ...
  // Live state, through lib/inplayState's measured multipliers. Absent or
  // unreported cards leave the frozen baseline exactly as it was.
  const { lambdaHome, lambdaAway } = adjustLambdaForCards(
    { lambdaHome: baseHome, lambdaAway: baseAway }, opts.liveState,
  );
  // The minimum-goals gate is tested on the ADJUSTED total, because that is
  // what the Over is priced against — testing the frozen one would admit a
  // fixture the card has since taken out of sniper territory, or refuse one it
  // has brought in.
  ...
  for (const line of lines) {
    ...
    if (!isBackablePrice(priced.over.odds, maxOdds)) continue; // longshot Over — model is not calibrated there
