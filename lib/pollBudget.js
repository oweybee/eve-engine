...
/**
 * Re-tier a persisted fixture_schedule against the current time.
 *
 * planDay.js assigns each fixture's tier once, when the plan is built. As the
 * day passes, a fixture drifts from far → near → dayof → closing, but nothing
 * ever revisits that assignment — a fixture tiered 'near' (3h) at plan time
 * stays on a 3-hour poll all the way through its actual closing window unless
 * planDay.js happens to re-run. This promotes (never demotes) any fixture
 * whose current hoursToKickoff now qualifies it for a tighter tier than the
 * one stored, and pulls nextPollAt forward to now so the promotion takes
 * effect on this run rather than waiting out the stale interval.
 *
 * Never widens a tier — only the daily budget planner is allowed to do that
 * (graceful degradation, cheapest information first). A promotion here is
 * always a fixture catching up on polling it should already have had.
 *
 * Entries with no kickoffAt (legacy plans written before it was persisted)
 * or a kickoffAt already in the past are left untouched — this is a
 * scheduling correction, not a fixture-lifecycle decision.
 *
 * @param {object} scheduleObj  plan.fixture_schedule: {id: {tier, everyMin, nextPollAt, kickoffAt}}
 * @param {object} opts
 *   tiers — override DEFAULT_TIERS
 *   now   — Date (defaults to now)
 * @returns {{schedule: object, promoted: Array<{id, from, to}>}}
 */
function retierSchedule(scheduleObj, { tiers = DEFAULT_TIERS, now = new Date() } = {}) {
  const nowMs = now.getTime();
  const schedule = { ...scheduleObj };
  const promoted = [];

  for (const [id, entry] of Object.entries(scheduleObj || {})) {
    if (!entry?.kickoffAt || !entry?.everyMin) continue;
    const hoursToKickoff = (new Date(entry.kickoffAt).getTime() - nowMs) / HOUR;
    if (!(hoursToKickoff > 0)) continue;

    const correct = tierFor(hoursToKickoff, tiers);
    if (!correct || correct.everyMin >= entry.everyMin) continue;

    const currentNextPollAt = entry.nextPollAt ? new Date(entry.nextPollAt).getTime() : nowMs;
    schedule[id] = {
      ...entry,
      tier: correct.key,
      everyMin: correct.everyMin,
      nextPollAt: new Date(Math.min(currentNextPollAt, nowMs)).toISOString(),
    };
    promoted.push({ id, from: entry.tier, to: correct.key });
  }

  return { schedule, promoted };
}
...
module.exports = { planPolling, pollsForFixture, tierFor, dueNow, retierSchedule, summarise,
                   DEFAULT_TIERS };
