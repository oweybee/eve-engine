...
/**
 * Live probability that the FINAL total goals exceed `line`, given the pre-match
 * per-side goal expectations, the current score and the minute — the engine
 * behind the Second Half Sniper.
 *
 * The remaining goals for the whole match are Poisson(λ_total · time-left), where
 * λ_total = λ_home + λ_away (the sum of two independent Poissons is Poisson of
 * the summed rate, so only the total rate matters for a total-goals market). The
 * final total is (goals already scored + remaining), hence
 *   P(final > line) = P(remaining ≥ ceil(line − currentTotal)).
 * A .5 line makes that threshold unambiguous. Goals already past the line settle
 * it at 1. Pure and deterministic — same maths family as liveWinProb.
 *
 * @param {object} p
 * @param {number} p.lambdaHome  full-match expected goals, home
 * @param {number} p.lambdaAway  full-match expected goals, away
 * @param {number} p.homeGoals   current home goals
 * @param {number} p.awayGoals   current away goals
 * @param {number} p.minute      elapsed minutes (≈45 at the half-time break)
 * @param {number} p.line        Over/Under goals line (e.g. 1.5, 2.5)
 * @returns {number} P(final total goals > line) in [0,1]
 */
function goalsOverProb({ lambdaHome, lambdaAway, homeGoals = 0, awayGoals = 0, minute = 0, line = 2.5 }) {
  const lambdaTotal = Math.max(0, (Number(lambdaHome) || 0) + (Number(lambdaAway) || 0));
  const current =
    Math.max(0, Math.round(Number(homeGoals) || 0)) +
    Math.max(0, Math.round(Number(awayGoals) || 0));
  if (current > line) return 1;                       // line already cleared
  const needed = Math.ceil(line - current);           // remaining goals required to go over
  if (needed <= 0) return 1;
  const f = timeLeftFraction(minute);
  const pmf = poissonPmfArray(lambdaTotal * f);       // remaining-goals distribution
  let cdfBelow = 0;                                   // P(remaining ≤ needed − 1)
  for (let k = 0; k < needed && k < pmf.length; k++) cdfBelow += pmf[k];
