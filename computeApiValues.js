...
function computeApiMatchEdge(match) {
  ...
  // Deduplicate: one row per bookmaker (latest fetched_at).
  ...
  // Probabilities stored as TEXT "0.5500" by parsePct() in fetchMatchDetails.js.
...
async function insertValueSignals(supabase, rows) {
  ...
  for (const row of rows) {
    for (const outcome of ['home', 'draw', 'away']) {
      ...
      // Derive signal tier: use fair_odds to recover p_api for longshot detection.
